...
export default function AddBrandPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const checkRes = await fetch("http://localhost:5000/api/brands");
      const data = await checkRes.json();
      const isNameExist = (data.brands as Brand[]).some(
        (b) => b.name.trim().toLowerCase() === name.trim().toLowerCase()
      );
      const isImageExist = (data.brands as Brand[]).some(
        (b) => b.image?.split("/").pop() === image.name
      );
  ...
  return (
    <main className="main-content flex justify-center p-6">
      ...
      <div className="w-full max-w-md">
        ...
        >
          <h2 className="text-2xl font-semibold mb-6 text-center">Thêm Thương Hiệu</h2>
          <input
            type="text"
