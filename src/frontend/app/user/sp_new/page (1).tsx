...
import "../../../public/css/new-products.css"; // <— CSS riêng cho trang NEW
...
export default function NewProductsPage() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`${API}/api/products`, {
        params: { page: 1, limit: 1000, search: "", status: "active", sort: "new" },
      });
  ...
  const displayedAll = useMemo(() => {
    const arr = [...products].filter(p => p.isActive)
  ...
  return (
    <main className="main-content products-page new-products">
      <div className="container">
        <nav className="breadcrumb">
          <Link href="/">Trang chủ</Link> / <span className="current">Hàng mới</span>
        </nav>

        <section className="product-content">
          <h2 className="product-title">
            <span className="title-logo-new" aria-hidden="true">🔥 NEW</span>
            SẢN PHẨM MỚI NHẤT
          </h2>
                  ...
                  return (
                    <div key={p._id} className="col-5-sp">
                      <div className="img-sp" style={{ position: "relative" }}>
                        {/* Badge NEW riêng trang này */}
                        <span className="badge-new" aria-label="Sản phẩm mới">NEW</span>
